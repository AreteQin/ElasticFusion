...
LiveLogReader::LiveLogReader(std::string file, bool flipColors, CameraType type)
  ...
  if (type == CameraType::OpenNI2)
    cam =
        new OpenNI2Interface(Resolution::getInstance().width(), Resolution::getInstance().height());
  else if (type == CameraType::RealSense)
    cam = new RealSenseInterface(
...
void LiveLogReader::getNext() {
  ...
  memcpy(
      ...
      cam->frameBuffers[bufferIndex].first.first,
      Resolution::getInstance().numPixels() * 2);
  memcpy(
      &decompressionBufferImage[0],
      cam->frameBuffers[bufferIndex].first.second,
      Resolution::getInstance().numPixels() * 3);
